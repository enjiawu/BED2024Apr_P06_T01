...
describe('Community Forum Controller', () => {
  ...
  describe('createComment', () => {
    it('should create a new comment and return it with status 201', async () => {
        ...
        const createdComment = { commentId: 4, postId, ...newCommentData, likes: 0, reports: 0, dateCreated: new Date() };
        Post.createComment.mockResolvedValue(createdComment);

        const req = { 
            params: { id: postId.toString() }, // Ensure postId is passed in req.params
            body: newCommentData
        };
        const res = {
            json: jest.fn(),
        ...
        expect(Post.createComment).toHaveBeenCalledWith(postId, newCommentData);
        expect(res.status).toHaveBeenCalledWith(201);
        expect(res.json).toHaveBeenCalledWith(createdComment);
    });
    ...
    it('should handle errors and return a 500 status', async () => {
        ...
        const req = { 
            params: { id: '1' }, // Ensure postId is included in req.params
            body: {}
        };
        const res = {
            json: jest.fn(),

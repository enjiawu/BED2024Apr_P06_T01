...
class User {
    ...
    try {
      ...
      const sqlQuery = `
                ...
                SET 
                    ...
                    location = @location, 
                    bio = @bio 
                    ${newUserData.profilePicture ? ', profilePicture = @profilePicture' : ''}
  ...
  static async changePassword(body) {
    ...
    const { recordset: [mUserInfo] } = await request.query('select * from Users where userId = @userId')
    if (!bcrypt.compareSync(body.currentPassword, mUserInfo.passwordHash)) {
      throw new Error("User Current Password error");
    }
    ...
    const mPassword = bcrypt.hashSync(body.newPassword, 10)
    request.input('passwordHash', mPassword)
    await request.query('update Users set passwordHash = @passwordHash where userId = @userId')
    mUserInfo.passwordHash = mPassword
    connection.close();
    return mUserInfo;
  }
}
